...
// Since |D| <= 8 * 128 = 1024, each quantized
// coefficient fits in a std::int16_t.
...
// k < F and l < F  (Python version)
class SquareMask : public FrequencyMask { ... };

// k + l < F        (MAM3 assignment)
...
\begin{lstlisting}
// CSR: three std::vector, no special member
// function written (rule of zero).
class SparseMatrix {
    ...
private:
    std::size_t rows_;
