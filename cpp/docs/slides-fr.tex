...
\begin{frame}{Organisation du code}
  ...
  \begin{tikzpicture}[node distance=0.5cm and 0.7cm]
    \node[box, minimum width=2.6cm] (io) {\code{image\_io}\\ \code{load\_image}, \code{save\_png}\\ {\tiny seule section qui voit stb}};
  ...
  \vspace{0.5em}
  \small
  Trois fichiers : \code{jpeg.hpp} (déclarations commentées), \code{jpeg.cpp} (définitions,
  mêmes 12 sections), \code{main.cpp} \quad|\quad espace de noms \code{jpeg} \quad|\quad
  compilation séparée, \code{-std=c++20 -Wall -Wextra -pedantic}
\end{frame}
...
g++ -std=c++20 -O2 -Wall -Wextra -pedantic jpeg.cpp main.cpp -o jpeg_csr
...
\end{lstlisting}
  ...
  Dans le dépôt, \code{make \&\& make test} construit aussi les 17 tests.
\end{frame}
