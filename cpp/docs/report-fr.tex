...
\begin{center}
  ...
  {\small Code source : \url{https://github.com/BnRomain/jpeg-compression}, dossier \code{cpp/submission/}}
\end{center}
...
source, rendu avec ce rapport, est commenté section par section : nous n'en citons ici que de
courts extraits.
...
Le code rendu tient en trois fichiers : \code{jpeg.hpp} déclare tous les types et toutes les
fonctions en documentant leurs invariants et les exceptions qu'ils peuvent lancer,
\code{jpeg.cpp} en donne les définitions dans le même ordre, et \code{main.cpp} contient le
programme. Les douze sections, de \code{Matrix8} à la ligne de commande, portent les mêmes
numéros dans les deux premiers fichiers. C'est la compilation séparée de la séance 2 : les deux
\code{.cpp} forment deux unités de traduction, compilées avec les options du cours
(\code{-std=c++20 -Wall -Wextra -pedantic}, plus \code{-O2}) puis liées. Tout est rangé dans
l'espace de noms \code{jpeg} ; les fonctions utilitaires d'une section sont placées dans un
espace de noms anonyme, invisible depuis l'autre fichier. La bibliothèque stb, qui lit et écrit
les fichiers PNG et JPEG, n'est incluse que par \code{jpeg.cpp} : le reste du programme ne
manipule que notre classe \code{Image}. Le dépôt garde en plus le même code éclaté en onze
modules, avec un \code{Makefile} qui construit \code{jpeg\_csr} et \code{run\_tests} à partir des
mêmes objets ; les deux versions produisent des fichiers identiques octet par octet. Le schéma
suit le trajet des données ; chaque boîte est un type ou une fonction du code.
...
\vspace{0pt}
\begin{lstlisting}
// jpeg.cpp section 9, anonymous namespace
class StbPixels {
public:
...
et mesure la couverture : 91\,\% des lignes du code sont exécutées par les tests et la
